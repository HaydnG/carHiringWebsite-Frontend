...
@Component({
  ...
  template: `
    <div class="modal-header">
      ...
      <div class="row">
        <h1 class="col-4" style="text-align: center;min-width: 140PX">Admin Edit User</h1><div class="col" style="    min-width: 160px;
    ...
    <div class="modal-body">
      <form [formGroup]="registrationForm" (ngSubmit)="onSubmit(registrationForm.value)">
        <div class="form-group" style="margin: 0 auto;width: 90%;">
          ...
          <div class="form-group row" *ngIf="!this.adminEdit && !this.userService.isAdmin">
            <div class="col">
              <input id="password" class="form-control" placeholder="Old Password" type="password" formControlName="oldpassword">
            </div>
          </div>
          <div class="form-group row">
            <div class="col">
              <input id="password" class="form-control" placeholder="New Password" type="password" formControlName="password">
              <div *ngIf="this.errors['password']" class="errorMessage" >{{this.passwordMessage}}</div>
            </div>
          </div>
          <div class="form-group row">
            <div class="col">
              <input id="password2" class="form-control" placeholder="Re-type New Password" type="password" formControlName="password2">
          ...
          <div class="form-group row">
            <div class="col">
              <button class="button btn btn-primary form-control" type="submit">Edit User</button>
...
export class AdminEditUserComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this.registrationForm.get('names').setValue(this.user.Names);
    this.registrationForm.get('email').setValue(this.user.Email);

    const dob = new Date(this.user.DOB * 1000);

    this.registrationForm.get('dob').setValue(new NgbDate(dob.getFullYear(), dob.getMonth() + 1, dob.getDate()));
  }
  ...
  onSubmit(userData): void {
    ...
    Object.keys(this.registrationForm.controls).forEach(key => {
      if (key === 'password' || key === 'password2' || key === 'oldpassword'){
        return;
      }
    ...
    if (errors.length > 0){
      ...
      this.userService.EditUser(userData.firstname, userData.names, userData.email, userData.password,
        (new Date(userData.dob.year, userData.dob.month - 1,
          userData.dob.day).getTime() / 1000) + '', this.user.ID + '', userData.oldpassword, newUser => {
